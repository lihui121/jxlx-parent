...
import com.jxlx.common.UserContext;
import com.jxlx.model.User;
import com.jxlx.service.AccountService;
...
//      modelMap.put("agencyList",  agencyService.getAllAgency());
    ...
    ResultMsg retMsg =  UserHelper.validate(account);//表单验证
    if (retMsg.isSuccess() ) {
...
//         return "/user/accounts/registerSubmit";
           return ResultMsg.successMsg("注册成功").toString();
      }else {
//        return "redirect:/accounts/register?" + ResultMsg.errorMsg("邮箱已被占用").asUrlParams();
         return ResultMsg.errorMsg("邮箱已被占用").toString();
      }
    }else {
...
//  }
  ...
  public String loginSubmit(HttpServletRequest req){
    ...
    User user =  accountService.auth(username, password);//判断用户并生成token
